...
local function CreateShadow(f, t, offset, thickness, texture)
	...
	local shadow = CreateFrame("Frame", nil, border, BackdropTemplateMixin and "BackdropTemplate")
...
local function CreateRollFrame()
	...
	tfade:SetGradient("VERTICAL", CreateColor(.1, .1, .1, 0), CreateColor(.25, .25, .25, 0))

	local status = CreateFrame("StatusBar", nil, frame, BackdropTemplateMixin and "BackdropTemplate")
	status:SetWidth(280)--狀態欄長短
	status:SetHeight(6)
	...
	frame.fsbind = bind
	local greed, greedtext = CreateRollButton(frame, "Interface\\Buttons\\UI-GroupLoot-Coin-Up", "Interface\\Buttons\\UI-GroupLoot-Coin-Highlight", "Interface\\Buttons\\UI-GroupLoot-Coin-Down", 2, GREED, "LEFT", bind, "RIGHT", 0, -2)
	...
	loot:SetPoint("LEFT", de, "RIGHT", 5, -1)
	loot:SetHeight(10)
	loot:SetWidth(200)
...
local function START_LOOT_ROLL(rollid, time)
	...
	f.fsbind:SetText(bop and "拾綁" or "裝綁")
...
anchor:SetScript("OnEvent", function(frame, event, addon)
	...
	anchor:SetPoint("CENTER", UIParent, anchor.db.x and "BOTTOMLEFT" or "BOTTOM", anchor.db.x or 300, anchor.db.y or 720)
end)
